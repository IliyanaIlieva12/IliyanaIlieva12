...
namespace tattoo.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await _userStore.SetUserNameAsync(Customer, Input.UserName, CancellationToken.None); ////USER NAME vs Email
                await _emailStore.SetEmailAsync(Customer, Input.Email, CancellationToken.None);

                var result = await _userManager.CreateAsync(Customer, Input.Password);//!!! CREATE in DB

                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");
                    await _userManager.AddToRoleAsync(Customer, "User"); //!!! dobavqne na rolya User
